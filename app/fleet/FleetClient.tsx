...
export default function FleetClient() {
    ...
    useEffect(() => {
        const fetchFleet = async () => {
            const data = await getStrapiData<any[]>('fleets', defaultFleetData, (strapiData) => {
                return strapiData.map((item: any) => {
                    const attrs = item.attributes || item;
                    // Map Strapi structure to our local structure
                    return {
                        id: item.id,
                        name: attrs.name,
                        status: attrs.status,
                        description: attrs.description?.split('\n') || [attrs.description],
                        heroImage: getStrapiImageUrl(attrs.heroImage?.data?.attributes?.url || attrs.heroImage?.url),
                        mainImage: getStrapiImageUrl(attrs.mainImage?.data?.attributes?.url || attrs.mainImage?.url),
                        sideView: getStrapiImageUrl(attrs.sideView?.data?.attributes?.url || attrs.sideView?.url),
                        topView: getStrapiImageUrl(attrs.topView?.data?.attributes?.url || attrs.topView?.url),
                        frontView: getStrapiImageUrl(attrs.frontView?.data?.attributes?.url || attrs.frontView?.url),
                        specs: Array.isArray(attrs.specs) ? attrs.specs : [
                            { label: "Engine", value: attrs.engine },
                            { label: "Cruise Speed", value: attrs.cruiseSpeed },
                            { label: "Range", value: attrs.range },
                            { label: "Wingspan", value: attrs.wingspan }
                        ].filter(s => s.value)
                    };
                });
            });

            if (data && data.length > 0 && JSON.stringify(data) !== JSON.stringify(defaultFleetData)) {
                setFleetData(data);
            }
        };
