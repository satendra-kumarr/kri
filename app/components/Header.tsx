...
export default function Header() {
  ...
  const [notifications, setNotifications] = useState<string>("Admissions 2025 Open • Scholarships Available • Visit Campus Today");

  useEffect(() => {
    const fetchNotifications = async () => {
      const data = await getStrapiData<string>('notifications', "Admissions 2025 Open • Scholarships Available • Visit Campus Today", (strapiData) => {
        if (!Array.isArray(strapiData)) return "Admissions 2025 Open • Scholarships Available • Visit Campus Today";
        const texts = strapiData.map((item: any) => {
          const attrs = item.attributes || item;
          return attrs.text || attrs.message || attrs.content || attrs.notification || attrs.Title || attrs.title || "";
        }).filter(Boolean);
        
        return texts.length > 0 ? texts.join(" • ") : "Admissions 2025 Open • Scholarships Available • Visit Campus Today";
      });
