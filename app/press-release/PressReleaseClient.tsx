...
const defaultPressCuttings: PressCutting[] = [
    {
        id: 1,
...
export default function PressReleaseClient() {
    ...
    const [pressCuttings, setPressCuttings] = useState<PressCutting[]>(defaultPressCuttings);

    useEffect(() => {
        const fetchPressReleases = async () => {
            const data = await getStrapiData<PressCutting[]>('press-releases', defaultPressCuttings, (strapiData) => {
                return strapiData.map((item: any) => {
                    const attrs = item.attributes || item;
                    
                    // Handle image extraction more robustly
                    const media = Array.isArray(attrs.image) ? attrs.image[0] : (attrs.image?.data?.attributes || attrs.image?.data || attrs.image);
                    const rawImageUrl = media?.url || media?.attributes?.url;
                    const image = rawImageUrl ? getStrapiImageUrl(rawImageUrl) : "/images/press.jpg";
                    console.log(`Press Release Item: ${attrs.title}, Image URL: ${image}`);

                    // Extract text from Strapi Rich Text blocks if description is an array
                    let descriptionText = "";
                    ...
                    return {
                        ...
                        date: attrs.date,
                        image,
                        category: attrs.Achieve || attrs.category || "General",
                        description: descriptionText
                    };
                });
            });

            if (data && data.length > 0 && JSON.stringify(data) !== JSON.stringify(defaultPressCuttings)) {
                setPressCuttings(data);
            }
        };
                            ...
                            >
                                ...
                                <div className="relative h-64 overflow-hidden bg-gray-100 cursor-zoom-in group-hover:bg-black/5" onClick={() => setSelectedImage(item)}>
                                    <Image
                                        ...
                                        fill
                                        unoptimized
                                        className="object-cover opacity-95 transition-all duration-700 group-hover:scale-105 group-hover:opacity-100"
                                    />
