...
const defaultPlacedStudents: PlacedStudent[] = [
    {
...
export default function TrainingPlacementClient() {
    ...
    const [placedStudents, setPlacedStudents] = useState<PlacedStudent[]>(defaultPlacedStudents);

    useEffect(() => {
        const fetchStudents = async () => {
            console.log("Fetching top-students...");
            const data = await getStrapiData<PlacedStudent[]>('top-students', defaultPlacedStudents, (strapiData) => {
                console.log("Raw Placed Students Data:", strapiData);
                if (!Array.isArray(strapiData)) {
                    console.warn("strapiData is not an array for top-students!");
                    return defaultPlacedStudents;
                }
                const transformed = strapiData.map((item: any) => {
                    const attrs = item.attributes || item;
                    
                    // Handle image extraction with support for Strapi 4 and 5
                    const media = Array.isArray(attrs.image) ? attrs.image[0] : (attrs.image?.data?.attributes || attrs.image?.data || attrs.image);
                    const imageUrl = media?.url ? getStrapiImageUrl(media.url) : "";

                    return {
                        title: attrs.name || attrs.title || "Student Name",
                        image: imageUrl || "/images/placeholder.jpg",
                        work: attrs.work || attrs.company || attrs.placement || attrs.job || "Aviation Professional",
                        batch: attrs.batch || attrs.year || "KRI Alumnus"
                    };
                });
                console.log("Transformed Placed Students:", transformed);
                return transformed;
            });
    ...
    const achievements = [
        {
            ...
            points: [
                ...
                "DMK & Central Government Leaders: Recognized initiatives in skill development and nation-building."
            ]
        },
        {
